...
def RTStruct(maskVolumeStruct, maskName, DICOMImageStruct, outputColor,fname='RS_test.dcm', flag_pos=0, flag_ori=0,
    ...
    print(DICOMImageStruct[0].ImagePositionPatient)
    ...
    print("LENGTH DICOM IMAGE STRUCT : ", len(DICOMImageStruct))
    for i in range(len(DICOMImageStruct)):
        item1 = Dataset()
    ...
    for nm in range(numROI):
        ...
        for numz in range(MaskVolume.shape[0]):  # for different z-axis
                ...
                for part_contour in contours:
                    ...
                    for index, i in enumerate(part_contour):
                        ...
                        if up == False and right == False and down == True and left == True:
                            ...
                            print("One Pixel Case, thin horizontally")
                        elif up == True and right == False and down == True and left == False:
                            print("One Pixel Case, thin vertically")
                        else:
            ...
            temp_ImageSequence.ReferencedSOPClassUID = DICOMImageStruct[nm].SOPClassUID
            temp_ImageSequence.ReferencedSOPInstanceUID = DICOMImageStruct[nm].SOPInstanceUID
            ...
            for i, temp_contours in enumerate(contours):  # for multiple (separate) structures in a single slice
                ...
                # If mask too small to have one or two dots in contours
                if len(temp_contours.shape) > 1:
                    ###### Indent <<
                    total_contours = np.concatenate((temp_contours, zlist), axis=1)

                    if total_contours.shape[0] > 0:
                        total_contours = total_contours * np.array([xspc, yspc, 1]) + np.array(
                            [maskpos_x, maskpos_y, 0])
                        ...
                        if len(ContourData) % 3 != 0:
                            print(len(ContourData), " is not multiple of 3!!")
                            break
                    ...
                    print(temp_contours, temp_contours.shape, temp_contours.dtype)
                    print(maskName[nm] + " is too small to contour for the current slice. Please check it!")
...
def writeDCMheader(filename, dcmStruct):
    ...
    # file_meta.MediaStorageSOPInstanceUID = "1.3.6.1.4.1.9590.100.1.2.52753693512490389439747654723971669885"
...
def postprocess(pred, num_volume):
    ...
    if num_object > num_volume:
        ...
        print(unique_volume)
        print("removed : ", unique_volume[-num_volume - 1] - 1)
...
if __name__ == '__main__':
    path = r'C:\Users\cecardenas\Desktop\rpa_cases\HN25'
    main(dicom_path=path, destination_path=path)
