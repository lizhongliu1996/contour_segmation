...
def voxel_z_closest_list(images, labels, start, roi_z, surface_cord, voxelsize, roi):
    ...
    for z in roi_z:
        ...
        gradct = find_gradct(next_voxel[0], next_voxel[1], voxelsize, roi)
        gradct_x = gradct[0]
        gradct_y = gradct[1]
...
def calc_tg(r, i, start, surface_cord, a, voxelsize, roi, w, i0):
...
def find_tg(L, start, surface_cord, a, voxelsize, roi, w, roi_z, i0):
    tg_list = []
    for i in range(len(L)):
        tg = calc_tg(L[i][0], L[i][1], start, surface_cord, a, voxelsize, roi, w, i0)
        tg_list.append(tg)
...
def assd(slices, target_label, voxelsize, a, SD, circles, seed, k, w, images, labels, organ_i, smooth=True, blur=False):
    ...
    t = find_tg(L, start, surface_cord, a, voxelsize, roi, w, roi_z, i0)
    if (blur):
